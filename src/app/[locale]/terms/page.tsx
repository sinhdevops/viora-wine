...
export default async function TermsPage({ params }: { params: Promise<{ locale: string }> }) {
	...
	const SECTIONS = [
		...
		{ title: t("section8_title"), content: t("section8_content") },
		{ title: t("section9_title"), content: t("section9_content") },
	];
	...
	return (
		<div className="min-h-screen bg-white">
			...
			<div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 lg:px-8">
				...
				{/* Intro */}
				<div className="border-brand-primary/20 bg-brand-primary/5 mb-8 rounded-2xl border p-5">
					<p className="text-[13px] leading-relaxed text-gray-700">{t("intro")}</p>
				</div>
				...
				<div className="mt-12 rounded-2xl bg-gray-50 p-6">
					...
					<ul className="mt-3 space-y-1 text-[13px] text-gray-600">
						...
						<li>
							{t("contact_hours_label")} <span className="font-medium">{t("contact_hours")}</span>
						</li>
					</ul>
				</div>
				...
				<div className="mt-8 flex items-center gap-6 border-t border-gray-100 pt-8">
					<Link href="/privacy-policy" className="text-brand-primary text-[13px] font-medium hover:underline">
						{t("back_privacy")}
					</Link>
