	...
	return (
		<div className="min-h-screen bg-white">
			...
			<div className="mx-auto max-w-360 px-4 py-10 sm:px-6 lg:px-8">
				{/* Header */}
				<div className="mb-6">
					<h1 className="text-xl font-semibold tracking-tight uppercase md:text-[28px]">{t("title")}</h1>
				...
				<div className="mb-6">
					...
					<p className="mt-1.5 text-sm text-gray-500">{t("services_subtitle")}</p>
				</div>

				<div className="mb-14 grid grid-cols-1 gap-6 sm:grid-cols-3">
					{SERVICES.map(({ icon: Icon, title, desc }) => (
						<div key={title} className="rounded-2xl bg-gray-50 p-6">
							<div className="bg-brand-primary/10 text-brand-primary mb-4 flex h-12 w-12 items-center justify-center rounded-xl">
								<Icon size={24} />
							</div>
				...
				<div className="rounded-2xl bg-gray-50 px-8 py-10 text-center">
					<h3 className="mb-2 text-[18px] font-semibold tracking-tight text-gray-900 uppercase md:text-[22px]">
						{t("cta_title")}
					</h3>
					<p className="mb-6 text-sm text-gray-500">{t("cta_subtitle")}</p>
					<Link
						href="/contact"
						className="bg-brand-primary inline-block rounded-lg px-8 py-3 text-sm font-bold text-white transition-colors hover:bg-[#a30000]"
					>
						{t("cta_button")}
					</Link>
				</div>
