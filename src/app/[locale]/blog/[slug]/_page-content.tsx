...
function ArticleBody({ html }: { html: string }) {
	return <div className="article-body wrap-break-word" dangerouslySetInnerHTML={{ __html: html }} />;
}
...
export default function NewsDetailPageContent({
	...
	return (
		<div className="min-h-screen bg-white">
			...
			<article>
				{/* ①  Title Section ────────────────────────────────────────────────── */}
				<div className="mx-auto max-w-360 px-4 pt-8 pb-6 sm:px-6 lg:px-8">
					{/* Category badge */}
					<span className="bg-brand-primary/10 text-brand-primary mb-4 inline-block rounded-full px-3 py-1 text-[11px] font-bold tracking-widest uppercase">
						{categoryLabel}
					</span>

					{/* Title styled as H1 but actual H1 is in server component for SEO */}
					<div className="mb-5 text-2xl leading-tight font-semibold tracking-tight text-gray-900 md:text-[36px] lg:text-[42px]">
						{newsItem.title[locale]}
					</div>
				...
				<div className="mx-auto max-w-360 px-4 pt-6 pb-10 sm:px-6 lg:px-8">
					<div
						className={hasToc ? "lg:grid lg:grid-cols-[1fr_300px] lg:gap-10 xl:grid-cols-[1fr_320px]" : ""}
					>
						{/* ── LEFT COLUMN: main content (70%) ─────────────────────────── */}
						<div className="min-w-0">
							{/* Mobile TOC – collapsible pill, hidden on desktop */}
							<TableOfContents headings={headings} tocTitle={t("toc_title")} mode="mobile" />
							...
							{/* Zalo CTA */}
							<ZaloCtaBlock title={t("zalo_cta_title")} buttonLabel={t("zalo_cta_button")} />
              ...
              /> */}
							...
							<div className="mt-10 flex flex-wrap items-center gap-2 border-t border-gray-100 pt-8">
								{TAGS.map((tag) => (
									<span
										key={tag}
										className="hover:border-brand-primary hover:text-brand-primary cursor-default rounded-full border border-gray-200 px-3 py-1 text-[12px] font-medium text-gray-500 transition-colors"
									>
										#{tag}
									</span>
								))}
							...
							<div className="mt-10 border-t border-gray-100 pt-8">
								<Link
									href="/blog"
									className="hover:text-brand-primary inline-flex items-center gap-2 text-[13px] font-semibold text-gray-500 transition-colors"
								>
									<ArrowLeft size={15} />
									{t("back_to_news")}
								</Link>
						...
						{hasToc && (
							<aside className="hidden lg:block">
								<div className="sticky top-35 space-y-6">
									<TableOfContents headings={headings} tocTitle={t("toc_title")} mode="desktop" />
								</div>
							</aside>
						)}
					</div>
			...
			{/* ⑦  FAQ Section ──────────────────────────────────────────────────── */}
			<FaqSection title={t("faq_title")} items={t.raw("faq_items") || DEFAULT_FAQ_ITEMS} />
				...
				<section className="border-t border-gray-100 bg-gray-50 py-12 md:py-16">
					<div className="mx-auto max-w-360 px-4 sm:px-6 lg:px-8">
						<div className="mb-8 flex items-center justify-between">
							...
							<Link
								href="/blog"
								className="text-brand-primary flex items-center gap-1 text-[12px] font-semibold hover:underline"
							>
								...
								<SwiperSlide key={item.id}>
									...
									>
										<div className="relative mb-4 aspect-3/2 w-full overflow-hidden rounded-xl bg-gray-100">
											...
											/>
											<span className="absolute top-3 left-3 rounded-full bg-white/90 px-2.5 py-0.5 text-[10px] font-bold tracking-widest text-gray-700 uppercase backdrop-blur-sm">
										...
										<h3 className="group-hover:text-brand-primary mb-2 line-clamp-2 text-[15px] leading-snug font-bold text-gray-900 transition-colors">
											{item.title[locale]}
										</h3>
										<div className="text-brand-primary flex items-center gap-1 text-[12px] font-semibold">
