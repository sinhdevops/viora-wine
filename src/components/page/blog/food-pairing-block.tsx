  ...
  const inner = (
    <div
      className={[
        ...
        item.href
          ? "border-gray-100 hover:border-brand-primary/30 hover:shadow-md cursor-pointer"
          : "border-gray-100",
      ].join(" ")}
    >
      ...
      {item.href && (
        <span className="mt-auto text-[11px] font-semibold text-brand-primary group-hover:underline">
          Xem sản phẩm →
        </span>
      )}
    </div>
...
export default function FoodPairingBlock({
  title,
  items,
}: FoodPairingBlockProps) {
  return (
