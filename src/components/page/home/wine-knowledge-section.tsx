	...
	return (
		<section className="bg-white">
			<div className="mx-auto max-w-360 px-4 sm:px-6 lg:px-8">
				{/* Header */}
				<h2 className="mb-1.5 text-xl font-semibold tracking-[0.05em] uppercase md:text-[28px]">
					{t("knowledge_title")}
				</h2>
				<p className="mb-8 text-sm text-gray-500 md:text-[15px]">{t("knowledge_subtitle")}</p>
				...
				<div className="hidden grid-cols-1 gap-6 sm:grid sm:grid-cols-3 md:gap-8">
					{events.map((event) => (
						...
						>
							...
							<h3 className="mb-1.5 text-[15px] font-bold text-gray-900 md:text-[16px]">{event.name}</h3>
							{event.description && (
								<p className="line-clamp-3 text-[13px] leading-relaxed text-gray-500 md:text-sm">
									{event.description}
								</p>
							)}
							<span className="text-brand-primary mt-3 inline-block text-[13px] font-semibold group-hover:underline">
